		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<strong>Data Gejala</strong>
					@if (Auth::user())
					<form name="create" method="post" action="{{route('gejala.create')}}" id="formCreate">
					{{ method_field('GET') }}
					{{ csrf_field() }}
					...
					<tbody>
						@foreach ($gejala as $gejala)
							<tr>
								...
								@if (Auth::user())
									<td align="right">
										<button type="submit" class="btn btn-primary btn-sm" form = 'formEdit{{$gejala->id }}'><i class="fa fa-edit"></i></button>
										<button type="submit" class="btn btn-danger btn-sm" form='formHapus{{ $gejala->id}}'><i class="fa fa-trash"></i></button>
										<form name="{{$gejala->id}}" method="post" action="{{ route('gejala.destroy', [$gejala->id]) }}" id="formHapus{{$gejala->id}}">
											{{csrf_field()}}
											{{ method_field('DELETE') }}
										</form>
										<form name="{{$gejala->id}}" method="post" action="{{route('gejala.edit', [$gejala->id])}}" id="formEdit{{$gejala->id}}">
											{{ method_field('GET') }}
											{{ csrf_field() }}
										</form>
									</td>
