	...
	<div class="col-md-9">
		<div class="panel panel-default">
			<div class="panel-heading">
				...
				@if (Auth::user())
				<form name="create" method="post" action="{{route('penyakit.create')}}" id="formCreate">
					{{ method_field('GET') }}
					{{ csrf_field() }}
					...
					<tbody>
						...
						@foreach ($penyakit as $penyakit)
							<tr>
								...
								@if (Auth::user())
								<td align="right">
										<button type="submit" class="btn btn-primary btn-sm" form='formEdit{{ $penyakit->id }}'><i class="fa fa-edit"></i></button>
										<button type="submit" class="btn btn-danger btn-sm" " form='formHapus{{ $penyakit->id }}'><i class="fa fa-trash"></i></button>
										<form name="{{ $penyakit->id }}" method="post" action="{{route('penyakit.edit',[$penyakit->id])}}" id="formEdit{{ $penyakit->id }}">
											{{ method_field('GET') }}
											{{ csrf_field() }}
										</form>
										<form name="{{ $penyakit->id }}" method="post" action="{{route('penyakit.destroy',[$penyakit->id])}}" id="formHapus{{ $penyakit->id }}">
											{{ method_field('DELETE') }}
											{{ csrf_field() }}
										</form>
									</td>
